...
export default function AccountForm({ onSubmit, onCancel, initialData, isEdit = false }: AccountFormProps) {
  ...
  const validateField = useCallback((name: keyof AccountFormData, value: any): string | undefined => {
    switch (name) {
      case 'account_name':
  ...
  const validateForm = useCallback((): FormErrors => {
    ...
    Object.keys(formData).forEach((key) => {
      const error = validateField(key as keyof AccountFormData, formData[key as keyof AccountFormData]);
      if (error) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border max-w-2xl mx-auto">
      <h2 className="text-xl font-semibold text-gray-900 mb-6">{isEdit ? "Edit Account" : "Add New Account"}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          />
          <p id="account_name_help" className="text-xs text-gray-500 mt-1">Give your account a memorable name</p>
          {errors.account_name && (
        ...
        <div>
          ...
          />
          <p id="account_number_help" className="text-xs text-gray-500 mt-1">For security, only enter the last 4 digits</p>
          {errors.account_number_masked && (
        ...
        <div>
          ...
          />
          <p id="routing_number_help" className="text-xs text-gray-500 mt-1">Optional - 9-digit number for ACH transfers</p>
          {errors.routing_number && (
        ...
        <div>
          ...
          />
          <p id="current_balance_help" className="text-xs text-gray-500 mt-1">Optional - leave blank if unknown. Negative values allowed for credit accounts.</p>
          {errors.current_balance && (
        ...
        {errors.general && (
          <div className="text-red-600 text-sm bg-red-50 p-3 rounded-md border border-red-200" role="alert">
            {errors.general}
          </div>
        )}
